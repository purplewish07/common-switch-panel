        ...
        execute: function () {
            ...
            commonSwitchPanelCtrl = /** @class */ (function (_super) {
                ...
                commonSwitchPanelCtrl.prototype.getLastDay = function (year, month) {
                    // 確保參數為數字型別
                    var numYear = typeof year === 'string' ? parseInt(year, 10) : year;
                    var numMonth = typeof month === 'string' ? parseInt(month, 10) : month;
                    // 添加調試信息
                    // console.log('getLastDay inputs:', { year, month, numYear, numMonth });
                    // 驗證參數有效性
                    if (isNaN(numYear) || isNaN(numMonth) || numMonth < 1 || numMonth > 12) {
                        console.error('Invalid parameters for getLastDay:', { numYear: numYear, numMonth: numMonth });
                ...
                commonSwitchPanelCtrl.prototype.todayFun = function () {
                    ...
                    if (this.panel.dayToShift && this.panel.dayToType) {
                        var toType = this.panel.dayToType;
                        var toData = this.panel.dayToShift;
                        if (toType === 'hour') {
                    ...
                    if (fromOffsetMs !== 0 || toOffsetMs !== 0) {
                        ...
                        if (toOffsetMs !== 0) {
                            ...
                            if (toOffsetMs > 0) {
                                ...
                                var effectiveOffsetMs = dayMs + toOffsetMs;
                        ...
                        if (this.firstload === false) {
                            this.timeSrv.setTime({
                                from: data_1.default.toUtc(beginDate.getTime()),
                                to: data_1.default.toUtc(finishDate.getTime())
                            });
                        }
                    }
                    else {
